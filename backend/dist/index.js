...
let possibilities = 0;
function generateChessboardImage(fen, filename) {
    const chess = new chess_js_1.Chess(fen);
    const canvas = (0, canvas_1.createCanvas)(boardSize, boardSize);
    const ctx = canvas.getContext("2d");
    // Draw the chessboard
    ctx.fillStyle = "#f0d9b5";
    ctx.fillRect(0, 0, boardSize, boardSize);
...
function saveChessboardImages(moves) {
    moves.forEach((move, index) => {
        const filename = `/media/deeks/New Volume/Projects/chess/backend/possibilities/chessboard_${index + 1}.png`;
        generateChessboardImage(move.after, filename);
        console.log(`Saved: ${filename}`);
...
function minimax(chess, depth, isMaximizing, alpha, beta) {
    if (depth === 0 || chess.isGameOver()) {
        return evaluateBoard(chess.board());
    }
    const moves = chess.moves({ verbose: true });
